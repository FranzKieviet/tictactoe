...
public class AI {
     ...
     *
     */
    public Pair<Integer, Integer> determineMove(Board board) {
        ...
        for (int i = 0; i < board.BOARD_SIZE; ++i) {
            for (int j = 0; j < board.BOARD_SIZE; ++j) {
                if (board.getBoardSpots()[i][j] == PlayType.NOTHING) {
                    ...
                    int eval = minimax(board, Integer.MAX_VALUE, Integer.MIN_VALUE, false);
     ...
     *
     */
    public int minimax(Board board, int scoreX, int scoreO, boolean maximizingPlayerTurn) {

        //Determine if game is over
        if (board.checkState() == winOpponent) return -1;        //-1 for loss
        if (board.checkState() == winAI) return 1;         // +1 for win
